...
export default function Card({ id, quest }) {
  ...
  return (
    <Link href={`quest/${id}`}>
      <a className='md:min-h-[13rem] h-full max-w-md w-full md:max-w-none mx-auto shadow-lg hover:shadow-xl active:shadow-lg z-10 min-h-full space-y-9 flex flex-col md:justify-between items-center hover:scale-101 md:active:scale-100 transition duration-75 rounded-md transform md:odd:-rotate-0.25 odd:hover:-rotate-1 md:even:rotate-0.25 even:hover:rotate-1 bg-gray-200 hover:bg-gray-100 text-black p-6 md:p-8 cursor-default'>
