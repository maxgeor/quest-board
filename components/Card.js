...
export default function Card({ id, quest }) {
  ...
  return (
    <Link href={`quest/${id}`}>
      <a className='flex flex-col justify-between items-center space-y-7 p-7 h-min min-h-[12rem] md:min-h-[14rem] max-w-md w-full md:max-w-none mx-auto shadow-lg hover:shadow-xl active:shadow-lg z-10 md:justify-between hover:scale-101 md:active:scale-100 transition duration-75 rounded-md transform md:odd:-rotate-0.25 odd:hover:-rotate-1 md:even:rotate-0.25 even:hover:rotate-1 bg-gray-200 hover:bg-gray-100 text-black cursor-default'>
        ...
        {quest.has_reward && <>
        <div className='bg-gray-300/50 rounded py-3 w-full flex justify-center font-serif space-x-1.5'>
