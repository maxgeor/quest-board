...
export default function Quest({ quest }) {
  return (
    ...
    <div className={`overflow-y-scroll z-20 h-[calc(100vh-32px)] md:h-[calc(100vh-48px)] w-full bottom-0 md:top-1/2 left-1/2 transform md:-translate-y-1/2 -translate-x-1/2 flex flex-col items-center bg-gray-200 text-gray-900 max-w-lg md:max-w-2xl rounded-lg rounded-b-0 md:rounded-b-lg shadow-2xl`}>
      <div className='relative md:py-4 px-4 md:px-12 h-full space-y-12'>
        <span className='cursor-pointer absolute top-3 md:top-5 right-3 md:right-5 z-20 transform rotate-45 text-4xl font-black h-6 w-6 text-gray-600'>+</span>
        <div className='space-y-6'>
          ...
          <div className='w-full text-xl flex items-center justify-center space-x-4'>
            <div className='text-sm font-mono text-center text-gray-600'><span className=''>{quest.fields.postedOn}</span></div>
        ...
        <article className='text- mt-96'></article>
      </div>
    </div>
  )
}
